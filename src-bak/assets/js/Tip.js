var Tip = function(){
	...
	if(document.querySelectorAll("#tip").length==0){
		...
		if(isMobile){
			style.appendChild(document.createTextNode('#tip {transition: opacity .5s;width: 100%;height: 100%;position: fixed;top: 0;left: 0;z-index: 999999;text-align: center;display: none;}#tip .tip-body {transition: all .3s;width: 0;/*width: 60%;*/height: 0;/*height: 20%;*/padding: 0;/*padding: 10%;*/display: inline-block;background-color: #fff;position: relative;top: 25%;border-radius: 5px;box-shadow: 0px 0px 5px #888888;font-size: 0.9rem;}#tip .tip-main{word-wrap: break-word;}#tip .tip-btns {width: 100%;height: 20%;position: absolute;left: 0;bottom: 0;}#tip .tip-btns button {color: #fff;border: 0;border-radius: 5px;padding: 4px 15px;cursor: pointer;}#tip .sure {background-color: rgb(18, 150, 219);margin-right: 10%;}#tip .cancel {background-color: rgb(204, 50, 47);margin-left: 10%;}#tip .tip-body>* {opacity: 0;}'));
			document.querySelector("head").appendChild(style);
		}else{
			style.appendChild(document.createTextNode('#tip {transition: opacity .5s;display: none;position: fixed;width: 100%;height: 100%;top: 0;left: 0;z-index: 999999;}#tip .tip-body {transition: all .3s;width: 0;/*width: 240px;*/height: 0;/*height: 140px;*/padding: 0;/*padding: 30px;*/position: absolute;top: 50%;left: 50%;transform: translateX(-50%) translateY(-50%);background-color: #fff;border-radius: 5px;box-shadow: 0 0 5px #888888;}#tip .tip-main{padding: 20px 10px;font-size: 16px;text-align: center;height: 100px;word-wrap: break-word;}#tip .tip-btns {position: absolute;width: 100%;height: 30px;left: 0;bottom: 10px;text-align: center;}#tip .tip-btns button {color: #fff;border: 0;border-radius: 5px;padding: 4px 15px;cursor: pointer;}#tip .sure {background-color: rgb(18, 150, 219);margin-right: 10%;}#tip .cancel {background-color: rgb(204, 50, 47);margin-left: 10%;}#tip .tip-body>* {opacity: 0;}'));
			document.querySelector("head").appendChild(style)
		}
